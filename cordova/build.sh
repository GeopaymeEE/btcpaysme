...
if [ ! -d $PROJECT ]; then
  ...
  cordova create project me.geopay.btcpays btcpay
  checkOK
...
if $DBGJS
then
  echo "${OpenColor}${Green}* Generating btcpays bundle (debug js)...${CloseColor}"
  cd $BUILDDIR/..
  grunt
  checkOK
else
  echo "${OpenColor}${Green}* Generating btcpays bundle...${CloseColor}"
  cd $BUILDDIR/..
  grunt prod
  checkOK
fi
